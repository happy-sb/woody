package happy2b.woody.agct.resource.fetch.inst;
...
public class ResourceFetcherTransformer implements ClassFileTransformer {
    ...
    ResourceFetcherTransformer(Set<String> resourceTypes) {
        this.resourceTypes = resourceTypes;
    }

    @Override
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) {
        if (!AGCTPredicate.acceptResourceFetching(loader, className) || !resourceTypes.contains(className)) {
            return null;
        }
